...
void CAN2_RX0_IRQHandler(void)
{
    ...
    if (CAN_GetITStatus(CAN2,CAN_IT_FMP0)!= RESET) 
    {
        ...
        CAN_Receive(CAN2, CAN_FIFO0, &Rx_message);
			  ...
			  angle=(int32_t)((uint32_t)(Rx_message.Data[0])<<24|(uint32_t)(Rx_message.Data[1])<<16|(uint32_t)(Rx_message.Data[2])<<8|(uint32_t)(Rx_message.Data[3])<<0);
    }
}

void SendGimbalPosition(uint16_t Gim_yaw,uint16_t Gim_pitch,uint8_t Shoot,uint8_t Mode)
{
...
void SendGimbalPosition_mouse(int16_t Gim_yaw,int16_t Gim_pitch,uint8_t Shoot,uint8_t Mode)
{
	  CanTxMsg tx2_message;
